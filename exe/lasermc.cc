/** @file lasermc.cc
    @brief Create MC laser events using real empty events
    @author agcocco
*/
...
int main(int argc, char** argv)
{
  ...
  config->AddCommandSwitch('x',"ser-rate","PMT SER rate",
  			   ...
  			   "Hz");
  config->AddCommandSwitch('m',"poisson-mean","Poisson Mean",
			   CommandSwitch::DefaultRead<double>(poisson_mean),
			   "n");
  ...
  EventHandler* modules = EventHandler::GetInstance();
  mclaserdata = modules->AddModule<MCLaserData>();
  ...
  mclaserdata->Start_Run();
  if(rand_seed > 0) mclaserdata->Set_Random_Seed(rand_seed);
  mclaserdata->Set_PoissonMean(poisson_mean);
