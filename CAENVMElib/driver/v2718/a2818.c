...
printk(KERN_INFO PFX "wait-ret = %d\n", ret );
        ...
        put_user(s->rx_status[slave], (int *)status);
...
#endif
                            a2818_reset(s);
                            udelay(10000);  // Wait 10 ms per ora
                            if( readl(s->baseaddr + A2818_STATUS) & A2818_LINK_FAIL ) {
                                        ret = -EFAULT;
...
/*
        ...
        ----------------------------------------------------------------------
*/
static void a2818_init_board(struct pci_dev *pcidev, int index)
{
        struct a2818_state *s;
        int i;
...
// Rev 1.5
#if LINUX_VERSION_CODE < VERSION(2,5,0)
                free_kiovec(MAX_V2718, s->il_buf);
#endif
                kfree(s);
// 1.4
                for( i = 0; i < MAX_V2718; i++ ) {
